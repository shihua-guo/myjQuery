...
public class Combine3 {
	...
	public void combine() {
		...
		try {
			...
			System.out.println("继续过滤完全匹配之后还能部分匹配的：");
			//过滤完全匹配还能部分匹配的
			// 存放完全匹配然后过滤的
			List<Object> filterDpiList = new ArrayList<>();
			int countFilter=0;
			for(int i=0;i<allMatchIcaList.size();i++){
				Object ica = allMatchIcaList.get(i);
				String nameSmallClassICA = ((ICA) ica).getNameSmallClass();
				// 判断是否包含
				for (int j = 0; j < dpiList.size(); j++) {
					...
					if (subClassNameDPI.indexOf(nameSmallClassICA) != -1) {
						...
						// 计数
						countFilter++;
						filterDpiList.add(dpi);
					}
				}
				//移除过滤的
				dpiList.removeAll(filterDpiList);
			}
			pw.println("-----------------------再次过滤完全匹配的");
			pw.flush();
			System.out.println("此部分一共有："+countFilter);
			...
			for (Object dpi : dpiList) {
				String beforeError = dpi.toString();
				//添加错误代码：业务类型错误
				String error = "99"+((DPI)dpi).getType();
				((DPI)dpi).setType(error);
				//子业务编码
				error = "99"+((DPI)dpi).getNum();
				((DPI)dpi).setNum(error);
				//存入文件
				pwDpiFail.println(beforeError+dpi.toString());
				pwDpiFail.flush();
